...
public class ClientObject
{
    ...
    public void ProcessActionCard(Packet packet)
    {
        ...
        switch (actionCard.ActionCard)
        {
            ...
            case "Skip":
                ...
                break;
            case "Defuse":
                ProcessDefuse(actionCard.ActionCard);
                break;
        }
    }
    public void ProcessDefuse(string cardName)
    {
        Game.Deck.Add(Cards.typeCards[CardType.ExplodingKitten]);
        Game.Shuffle();
    ...
    public void ProcessTakeCard()
    {
        ...
        {
            QueuePacketSend(PacketConverter.Serialize(PacketType.ExplodingKitten, new PacketExplodingKitten() { State = State.Play }).ToPacket());
            SendPacketPlayerState(lastCardName, false);
            /*var otherClient = server._clients.Where(c => c.Id != Id).First();
            otherClient.QueuePacketSend(PacketConverter.Serialize(PacketType.ExplodingKitten, new PacketExplodingKitten() { State = State.Wait }).ToPacket());*/
        }
    }
